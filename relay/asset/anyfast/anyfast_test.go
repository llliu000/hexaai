...
func TestAnyfastAssetFullFlow(t *testing.T) {
	apiKey := os.Getenv("ANYFAST_TEST_API_KEY")
	if apiKey == "" {
		t.Skip("set ANYFAST_TEST_API_KEY to run integration test")
	}
	baseURL := os.Getenv("ANYFAST_TEST_BASE_URL")
	if baseURL == "" {
		baseURL = "https://www.anyfast.com.cn"
	}

	adaptor := Adaptor{ApiKey: apiKey, BaseURL: strings.TrimRight(baseURL, "/")}
	...
	assetURL := os.Getenv("ANYFAST_TEST_ASSET_URL")
	if assetURL == "" {
	...
	getAssetResp, err := adaptor.GetAsset(&dto.GetAssetRequest{
		BaseAssetRequest: dto.BaseAssetRequest{
			ProjectName: ptr(createGroupResp.Id),
		},
