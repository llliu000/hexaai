...
type TaskAdaptor struct {
	...
	apiKey       string
	baseURL      string
	organization string // 暂时用于同类型模型的不同渠道选择
}
...
func (a *TaskAdaptor) EstimateBilling(c *gin.Context, info *relaycommon.RelayInfo) map[string]float64 {
	...
	req, err := a.convertToRequestPayload(&taskReq)
	if err != nil {
		return nil
	}
	key := fmt.Sprintf("condition-%s", req.Model)
	ratio := priceRatio(req)
	return map[string]float64{key: ratio}
}
